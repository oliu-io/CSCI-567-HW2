...
class sequential(object):
    ...
    def apply_l1_regularization(self, lam):
        ...
        for layer in self.layers:
            for n, v in layer.grads.items():
                ######## TODO ########
                self.grads[n] += lam * np.sign(self.params[n])
                ######## END  ########
...
class gelu(object):
    ...
    def backward(self, dprev):
        ...
        if feat is None:
            raise ValueError("No forward function called before for this module!")
        dfeat = None
        z = np.sqrt(2/np.pi) * (feat + 0.044715 * (feat ** 3))
        dx = 0.5 * (1 + np.tanh(z)) + \
                    0.5 * feat * (1 - (np.tanh(z) ** 2)) * np.sqrt(2/np.pi) * (1 + 3 * 0.044715 * (feat ** 2))
        dfeat = dx * dprev
        self.meta = None
        return dfeat
